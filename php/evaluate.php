<?php
  ...
  if(!isset($_POST['str']))
      return;
  $st = explode('|',$_POST['str']); //hàm xử lý chuỗi
  $sname = array();
  ...
  for ($i=0; $i < sizeof($st); $i++) { 
  		$ind = strpos($st[$i], ','); // tìm vị trí xuất hiện đầu tiên của chuỗi cho trước
  		$sname = substr($st[$i],0,$ind); // cắt chuỗi $st[$i] bắt đầu từ 0, độ dài là $ind
  		$fv = substr($st[$i],$ind+1,strlen($st[$i])); // cắt 
      ...
      //lấy dữ liệu từ localhost xuống giao diện
  }
  ...
  $results = []; // lưu kết quả dưới dạng Object
